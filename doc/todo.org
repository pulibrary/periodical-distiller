...
*** TODO [#B] ~MediaDownloader~ created by ~PIPAggregator~ is never closed
:PROPERTIES:
...
*** TODO [#C] ~MediaDownloader._build_media_url~ (singular) is dead code
:PROPERTIES:
...
*** TODO [#B] ~PACKAGE_ROOT~ computed via four ~.parent~ calls — fragile
:PROPERTIES:
...
*** TODO [#C] Image DPI (150) is below archival standard
:PROPERTIES:
...
*** TODO [#C] ~_copy_article_media~ return value is unused
:PROPERTIES:
...
*** TODO [#C] ~_transform_article~ parameter ~pip_article~ is untyped
:PROPERTIES:
...
*** TODO [#B] ~Filter.process_token~ / ~validate_token~ should be ~@abstractmethod~
:PROPERTIES:
...
*** TODO [#B] ~Pipe.out_path~ / ~marked_path~ / ~error_path~ guard on wrong variable
:PROPERTIES:
...
*** TODO [#C] Individual pipeline filters have significant code duplication
:PROPERTIES:
...
*** TODO [#C] ~_find_token~ import inside method body
:PROPERTIES:
